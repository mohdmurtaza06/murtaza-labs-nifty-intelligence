...
function advancedMarketAnalytics(rows = []) {
  ...
  const dayChangePct = lastClose != null && previousClose != null && previousClose !== 0
...
export default function App() {
  ...
  const dashboardDayChange = Number(dashboard?.day_change_pct);
  const dayChangePct = Number.isFinite(dashboardDayChange)
    ? dashboardDayChange
    : (Number.isFinite(advanced.dayChangePct) ? advanced.dayChangePct : null);
          ...
          <div className="command-grid">
            <article className="command-hero-card reveal">
              ...
              <div className={`command-change ${directionClass(dayChangePct)}`}>
                {Number.isFinite(dayChangePct) ? `${dayChangePct >= 0 ? "+" : ""}${dayChangePct.toFixed(2)}% DAY CHANGE` : "DAY CHANGE UNAVAILABLE"}
              </div>
                  ...
                  <div className="cross-market-cell" key={label}>
                    ...
                    <strong className={suffix === "%" ? directionClass(Number(value)) : ""}>
                      {value !== null && value !== undefined && value !== "" && Number.isFinite(Number(value))
                        ? `${(Number(value) * (suffix === "%" ? 100 : 1)).toFixed(2)}${suffix}`
                        : "—"}
                    </strong>
                  </div>
                ))}
            ...
            <article className="terminal-chart-panel reveal">
              <div className="panel-header">
                ...
                <div className="terminal-price-tag">
                  <b>{nifty}</b>
                  <span className={directionClass(dayChangePct)}>{Number.isFinite(dayChangePct) ? `${dayChangePct >= 0 ? "+" : ""}${dayChangePct.toFixed(2)}%` : "—"}</span>
                </div>
              </div>
